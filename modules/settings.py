...
class Settings(commands.Cog):
    ...
    async def setticketchannel(self, ctx):
        ...
        if(isPremium):
            ...
            if ticketchanint == -1:
                await self.bot.sendSuccess(ctx, f"Create ticket channel is now unbounded", ctx.message, ctx.guild)
            else:
                await self.bot.sendSuccess(ctx, f"Create ticket channel is now {ticketchan.content}", [ctx.message, message], ctx.guild)
    ...
    async def setlogchannel(self, ctx):
        ...
        if(isPremium):
            ...
            if logchanint == -1:
                await self.bot.sendSuccess(ctx, f"Log channel is now unbounded", ctx.message, ctx.guild)
            else:
                await self.bot.sendSuccess(ctx, f"Log channel is now {logchan.content}", [ctx.message, message], ctx.guild)
    ...
    async def setticketprefix(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.sendSuccess(ctx, f"Ticket Prefix is now set to `{ticketprefix.content}`", [ctx.message, message], ctx.guild)
    ...
    async def setwelcomemessage(self, ctx):
        ...
        if(isPremium):
            try:
                message = await ctx.send("Please enter what you would like the welcome message to be for new tickets. **NOTE**: Must be 2000 characters or less!")
    ...
    async def setmaxtickets(self, ctx):
        ...
        if(isPremium):
            ...
            if tickenchanint == -1: 
                await self.bot.sendSuccess(ctx, f"Ticket Count is now set to `Unlimited`", [ctx.message, message], ctx.guild)
            else:
                await self.bot.sendSuccess(ctx, f"Ticket Count is now set to `{ticketchanint}`", [ctx.message, message], ctx.guild)
    ...
    async def setprefix(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.sendSuccess(ctx, f"Command Prefix is now set to `{prefix.content}`", [ctx.message, message], ctx.guild)
    ...
    async def setdmonjoin(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.sendSuccess(ctx, f"DM On Join is now set to `{dmonjoindata}`", [ctx.message, message], ctx.guild)
    ...
    async def setcleannew(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.sendSuccess(ctx, f"Clean New Ticket Invocations is now set to `{cleannewdata}`", [ctx.message, message], ctx.guild)
    ...
    async def setcleanall(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.sendSuccess(ctx, f"Clean All Invocations is now set to `{cleanalldata}`", [ctx.message, message], ctx.guild)
    ...
    async def setenforcesubject(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.sendSuccess(ctx, f"Enforcing Subject is now set to `{enforcesubjectdata}`", [ctx.message, message], ctx.guild)
    ...
    async def settranscripts(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.db.execute("UPDATE settings SET cleanall = $1 WHERE serverid = $2;", transcriptsdata, ctx.guild.id)
            await self.bot.sendSuccess(ctx, f"Sending Transcripts is now set to `{transcriptsdata}`", [ctx.message, message], ctx.guild)
    ...
    async def setadminclose(self, ctx):
        ...
        if(isPremium):
            ...
            await self.bot.db.execute("UPDATE settings SET cleanall = $1 WHERE serverid = $2;", adminclosedata, ctx.guild.id)
            await self.bot.sendSuccess(ctx, f"Admin Closing Tickets is now set to `{adminclosedata}`", [ctx.message, message], ctx.guild)
